...
public class CartasJson : MonoBehaviour
{
    ...
    IEnumerator FusionarConSlider(GameObject carta1, GameObject carta2)
    {
        ...
        if (cartaComp1.cartaEnum == CardEnum.Cocina && cartaComp2.cartaEnum == CardEnum.Maiz)
        {
            tipoFusion = CardEnum.Palomitas;
            duration = 5f;
            ResultadoFusion = cartaComp2.ResultadoFusion;
        }
        ...
        {
            slider.value = 0f;
            slider.gameObject.SetActive(true); // Aseguramos que est� visible

            while (elapsed < duration)
            {

                #region FusionCancelada
                if (fusionCancelada) // Si la fusi�n ha sido cancelada, detenemos la coroutine
                {
                    ...
                    cartasEnFusion.Remove(carta1); // Eliminar las cartas del HashSet si se cancel� la fusi�n
